...
public class NortGame {
	...
	/**
	 * Checks the current situation of the board and returns said
	 * NortGameSituation
	 * 
	 ...
	 * 
	 * @param player
	 *            The player that is being advanced
	 */
	...
	public NortGameCell getNextCell(Player player) {
		switch (player.getDirection()) {
			case LEFT:
				if (player.getCharX() - 1 > -1) return NortGameLoop.getInstance().getBoard()[player.getCharY()][player.getCharX() - 1];
				
				break;
			case RIGHT:
				if (player.getCharX() + 1 < NortGameLoop.getInstance().getBoard()[0].length) 
					return NortGameLoop.getInstance().getBoard()[player.getCharY()][player.getCharX() + 1];
				
				break;
			case UP:
				if (player.getCharY() - 1 > -1) return NortGameLoop.getInstance().getBoard()[player.getCharY() - 1][player.getCharX()];
				
				break;
			case DOWN:
				if (player.getCharY() + 1 < NortGameLoop.getInstance().getBoard().length) 
